import type { Menu, MenuSection, MenuItem, Product, ProductVariant, ModifierOption } from '@prisma/client';
type MenuProductRef = Pick<Product, 'id' | 'name' | 'type' | 'priceCents' | 'isActive' | 'isAvailable'> & {
    imageUrl: string | null;
    hasImage: boolean;
};
type MenuProductBasic = Pick<Product, 'id' | 'name' | 'type' | 'isActive' | 'isAvailable' | 'priceCents'> & {
    imageUrl: string | null;
    hasImage: boolean;
};
...
export type MenuSectionWithItems = MenuSection & {
    items: MenuItemWithRef[];
};
