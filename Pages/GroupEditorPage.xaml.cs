        ...
        try
        {
            ...
            else
            {
                var groupDto = new ModifiersApi.UpdateGroupDto
                {
                    ...
                    isRequired = ReqSwitch.IsToggled,
                    isActive = isActive,
                    position = position,                       // 👈 ahora sí
                    appliesToCategoryId = appliesToCategoryId  // 👈 ahora sí
                };
                await _api.UpdateGroupAsync(GroupId, groupDto);

                var opts = Options.Select(o => new ModifierOptionDTO
                {
                    id = o.id,
                    groupId = GroupId,
                    name = o.name?.Trim() ?? "",
                ...
                await _api.ReplaceGroupOptionsAsync(GroupId, opts);
